...
def ffmpeg_missing_message():
    return (
        "流式 MP3 导出失败：未找到 FFmpeg。\n"
        "推理通常已完成，但在将结果编码为 MP3 时失败。pydub 需要系统已安装 FFmpeg，且 `ffmpeg` 命令可在 PATH 中调用。\n"
        "解决方法：\n"
        "1. 安装 FFmpeg（Windows 可用 `winget install Gyan.FFmpeg` 或 `choco install ffmpeg`）\n"
        "2. 安装后重新打开终端并重启 Web UI\n"
        "3. 在命令行运行 `ffmpeg -version` 确认可用"
    )


def export_mp3_bytes(output_wave):
    ...
    try:
        ...
        raise gr.Error(ffmpeg_missing_message()) from exc
    except Exception as exc:
        raise gr.Error(
            "流式 MP3 导出失败。\n"
            f"错误类型：{type(exc).__name__}\n"
            f"错误信息：{exc}\n"
            "请确认 FFmpeg 已安装，并查看启动 Web UI 的终端日志。"
        ) from exc


def voice_conversion_with_error_handling(source, target, diffusion_steps, length_adjust, inference_cfg_rate, auto_f0_adjust, pitch_shift):
    if not source:
        raise gr.Error("请先上传或选择源音频（Source Audio / 源音频）。")
    if not target:
        raise gr.Error("请先上传或选择参考音频（Reference Audio / 参考音频）。")
    if not os.path.isfile(source):
        raise gr.Error(f"源音频文件不存在或不可读：{source}")
    if not os.path.isfile(target):
        raise gr.Error(f"参考音频文件不存在或不可读：{target}")
    if find_ffmpeg() is None:
        raise gr.Error(ffmpeg_missing_message())

    try:
        yield from voice_conversion(
        ...
        if getattr(exc, "winerror", None) == 2 or "ffmpeg" in str(exc).lower():
            raise gr.Error(ffmpeg_missing_message()) from exc
        raise gr.Error(f"文件未找到：{exc}") from exc
    except Exception as exc:
        ...
        raise gr.Error(
            ...
            f"错误类型：{type(exc).__name__}\n"
            f"错误信息：{exc}\n"
            "详细堆栈已输出到启动 Web UI 的终端，请查看服务端日志。"
        ) from exc
    ...
    while processed_frames < cond.size(1):
        ...
        if processed_frames == 0:
            if is_last_chunk:
                ...
                mp3_bytes = export_mp3_bytes(output_wave)
...
def main(args):
    ...
    if find_ffmpeg() is None:
        description += (
            "<br><br><b>警告 / Warning:</b> 未检测到 FFmpeg，流式 MP3 输出会失败。"
            "请安装 FFmpeg 并确保 `ffmpeg` 在 PATH 中可用。"
            "<br>FFmpeg was not found. Streaming MP3 output will fail until FFmpeg is installed and available in PATH."
        )
        print("WARNING: FFmpeg not found in PATH. Streaming MP3 export will fail.")
    inputs = [
    ...
    gr.Interface(fn=voice_conversion_with_error_handling,
