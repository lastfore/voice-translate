...
`merge-audio.py` 设计接口如下（`merge-audio.bat` 按同名参数解析并加引号后调用，可正确处理分离产物路径中的括号，如 `song_(Instrumental)_mel_band...`）：
...
| `scripts/convert-slices.bat` | ✅ 已实现 | 阶段 3：批量歌声转换（见 Seed-VC 指南 §5.4） |
| `scripts/convert-slices.py` | ✅ 已实现 | 批量转换核心逻辑 |
| `scripts/slice-vocals-lrc.bat` | ✅ 已实现 | LRC 切片（见词曲分离指南 §5.4） |
| `scripts/process-song.bat` | ✅ 已实现 | 阶段 1+2 一键：分离 + LRC 切片 |
| `scripts/slice-vocals.py` | ✅ 已实现 | VAD 切片 + `manifest.json` 输出 |
| `scripts/slice-vocals.bat` | 已有 | VAD 切片入口 |
