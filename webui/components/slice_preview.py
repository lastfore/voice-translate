...
from webui.components.mode_panel import SLICE_VAD
...
def wire_slice_preview(
    ...
    def _normalize_mode(mode: str | None) -> str:
        return paths.normalize_slice_mode(mode or SLICE_VAD)

    def _refresh(pid: str | None, mode: str | None):
        return slice_preview_values(pid, _normalize_mode(mode))

    def _on_row_select(pid: str | None, mode: str | None, evt: gr.SelectData):
        if not pid or evt.index is None:
            return None
        smode = _normalize_mode(mode)
        row_idx = evt.index[0] if isinstance(evt.index, (list, tuple)) else evt.index
        rows, _, _ = load_slice_table(pid, smode, include_tune_status=False)
        if row_idx < 0 or row_idx >= len(rows):
            return None
        slice_id = str(rows[row_idx][0])
        return audio_if_exists(audio_for_slice(pid, smode, slice_id))

    def _show_dir(pid: str | None, mode: str | None):
        if not pid:
            return "*请先选择项目*"
        mode_dir = paths.resolve_slices_mode_dir(pid, _normalize_mode(mode))
        if mode_dir is None:
            return "*无切片目录*"
